...
export default function SignUpPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: { data: { username: form.username.toLowerCase().replace(/[^a-z0-9_]/g, ''), display_name: form.display_name || form.username } },
    })
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px 24px' }}>
        <div style={{ width: '100%', maxWidth: 400 }}>
          <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <AuthField label="Username" type="text" value={form.username} onChange={set('username')} placeholder="your_name" autoComplete="username" required />
            <AuthField label="Display name" type="text" value={form.display_name} onChange={set('display_name')} placeholder="Your Full Name" autoComplete="name" />
            ...
            <AuthField label="Password" type="password" value={form.password} onChange={set('password')} placeholder="at least 8 characters" autoComplete="new-password" required />
          ...
          <p style={{ fontFamily: 'var(--sans)', fontSize: 13, color: 'var(--ink-mute)', marginTop: 16, lineHeight: 1.5, textAlign: 'center' }}>
          ...
          <div style={{ marginTop: 20, textAlign: 'center' }}>
            <p style={{ fontFamily: 'var(--sans)', fontSize: 14, color: 'var(--ink-mute)', margin: 0 }}>
              ...
              <Link href="/signin" style={{ color: 'var(--ink)', fontWeight: 500, textDecoration: 'none', borderBottom: '1px solid var(--rule)' }}>
                Sign in
              </Link>
            </p>
          </div>
